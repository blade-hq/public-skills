...
- NPM：`@blade-hq/agent-client`、`@blade-hq/agent-react`
...
如果当前工作目录不是本仓库，先定位本技能目录，再运行其中的脚本。脚本使用 inline dependency metadata 声明 `pydantic` 和 `pyyaml`，`uv run --script` 会自动准备依赖。这个脚本不替代平台加载校验，但能在 AI 写文件后快速发现常见结构错误。
