...
如果你不打算在自己的页面里渲染聊天组件，而是把 **Blade 的聊天页面整个用 iframe 嵌进自己的系统**，宿主页面用 `connectEmbedded()` 拿到与同页形态完全一样的协作 API。
...
| `iframe` | 否 | 嵌入 Blade 页面的 iframe 元素。不传则只监听来自任意子窗口的消息，且无法向智能体发送消息（`attach` / `insertText` / `send` 会失效并告警） |

返回的 `EmbeddedChat`：`onCommand(action, handler)`、`attach(label, data)`、`insertText(text)`、`send(text)`、`dispose()`。

## 安全说明（重要）

- **`allowedOrigins` 必填，不能省略、不能填 `"*"`**。不校验来源意味着任意第三方页面都能向你的页面伪造智能体指令（例如伪造一条"删除全部数据"的指令让你的页面执行）。SDK 在 `allowedOrigins` 为空时直接抛错。
- SDK 同时做两层校验：消息来源 origin 必须在允许列表内；传了 `iframe` 时还要求消息确实来自该 iframe 的窗口。
- 发往 iframe 的消息不使用 `"*"` 作为目标来源，避免消息内容泄漏给被替换的页面。
- 只把你真正信任的 Blade 部署地址放进列表；**不要**把用户可配置的地址原样传进来。
- 指令处理函数里不要直接 `eval` 或按 payload 拼接执行任意逻辑，按 action 白名单分发到具体的业务函数。

## action 从哪里来

与同页形态一致：action 是**技能作者**在工具返回值 JSON 的 `_meta.bridge` 里写的契约字符串，Blade 页面检测到自己处于 iframe 中时会把它转发给宿主页面。详见 [page-collaboration.md](page-collaboration.md)。
